...
#                sampled for each voxel during creation of the pattern data
  ...
  tau.cov <- matrix(c(tau.alpha^2,rho,rho,tau.beta^2),nrow=2)
  ...
  #Option: incorporate attentional sharpening
  if(boostGain==T){
    #Rank voxels on strength of experimental effect
    vox.rank <- as.data.frame(rfx)
    names(vox.rank) <- c('beta.0','beta.1')
    vox.rank$vox.id <- 1:nvox
    vox.rank$abs.beta.1 <- abs(vox.rank$beta.1)
    vox.rank <- dplyr::arrange(vox.rank,beta.1)
    pickvox <- round(nvox*.1,1)
    vox.boost <- vox.rank$vox.id[1:pickvox]
  }
  ...
  for(run in 1:nrun){
    ...
    for(v in 1:nvox){
      ...
      if(boostGain==T){
        if(v %in% vox.boost){
          betahats.loop <- betahats.loop * 5
        }else{
          betahats.loop <- betahats.loop * .25
        }
      }
...
# Tie both functions together and run a simulated subject -----------------
classify.subject <- function(nvox,ntrials,nrun,etvs,mu.alpha,mu.beta,gamma.run,
                             tau.alpha,tau.beta,rho,boostGain,
                             costVal,meanCtrRuns,avgTrial){
  # simulate the trial-by-trial data
  simulated.data <- simulate.betas(nvox,ntrials,nrun,
                                   etvs,
                                   mu.alpha,mu.beta,
                                   tau.alpha, tau.beta,rho,
                                   gamma.run,
                                   boostGain)
  #Concatenate patterns from all simulated runs
  ...
  simulated.result <- classify.betas(betas=simulated.data,labToClassify = 'cond',
                                     costVal, meanCtrRuns, avgTrial)
  # Extract mean CV classification accuracy
  mean.CA <- simulated.result[[1]]$meanAccuracy
  return(mean.CA)
}
